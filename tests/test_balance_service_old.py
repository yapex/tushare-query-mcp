"""
BalanceService测试用例
测试资产负债表服务的完整业务逻辑，包括数据获取、缓存、过滤和字段选择
"""
...
from tushare_query_mcp.schemas.request import BalanceRequest
from tushare_query_mcp.schemas.response import BalanceResponse, ResponseStatus
# 导入待测试的模块
...
class TestBalanceService:
    ...
    def mock_tushare_source(self):
        ...
        # 配置 get_balance_data 方法返回数据而不是协程
        mock_source.get_balance_data = AsyncMock(return_value=[])
    ...
    @pytest.fixture
    def balance_service(self, mock_config, mock_tushare_source, mock_cache):
        """创建BalanceService实例"""
        with patch(
        ...
        return [
            {
                ...
                "total_fix_assets": 300000000000.00,
                "total_liab": 500000000000.00,
                "total_cur_liab": 300000000000.00,
                "total_ncl": 200000000000.00,
                "money_cap": 100000000000.00,
                "capital_rese": 80000000000.00,
                "surplus_rese": 400000000000.00,
                "undistr_porfit": 1200000000000.00,
            ...
            },
            {
                ...
                "comp_type": "1",
                "end_type": "4",
                "total_assets": 1800000000000.00,
                "total_cur_assets": 1400000000000.00,
                "total_fix_assets": 280000000000.00,
                "total_liab": 450000000000.00,
                "total_cur_liab": 280000000000.00,
                "total_ncl": 170000000000.00,
                "money_cap": 100000000000.00,
                "capital_rese": 80000000000.00,
                "surplus_rese": 370000000000.00,
                "undistr_porfit": 1070000000000.00,
                "total_equity": 1350000000000.00,
                "update_flag": "1",
            },
        ]
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_cache_hit(
        self, balance_service, mock_cache, sample_raw_data, sample_request
    ):
        """测试缓存命中场景"""
        # 设置缓存返回数据
        ...
        assert response.data_type == "balance"
        ...
        assert selected_record["total_assets"] == 2000000000000.00

        # 验证查询时间很短（缓存命中）
        assert response.query_time < 0.01

    @pytest.mark.asyncio
    async def test_get_balance_data_cache_miss(
        self,
        balance_service,
        mock_cache,
        mock_tushare_source,
        ...
        # 验证数据源被调用
        mock_tushare_source.get_balance_data.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_with_update_flag_filtering(
        self, balance_service, mock_cache, mock_tushare_source
    ):
        """测试update_flag过滤功能"""
        # 设置包含重复记录的原始数据
        ...
        request = BalanceRequest(
            ts_code="600519.SH", fields=["end_date", "total_assets", "update_flag"]
        )

        # 调用服务
        response = await balance_service.get_balance_data(request)
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_field_selection(
        self, balance_service, mock_cache, sample_raw_data, sample_request
    ):
        """测试字段选择功能"""
        mock_cache.get.return_value = sample_raw_data
        ...
        # 验证字段值正确
        assert response.data[0]["total_assets"] == 2000000000000.00
        assert response.data[0]["total_liab"] == 500000000000.00

    @pytest.mark.asyncio
    async def test_get_balance_data_no_dates(
        self, balance_service, mock_cache, sample_raw_data
    ):
        """测试不提供日期范围"""
        mock_cache.get.return_value = sample_raw_data
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_empty_fields(
        self, balance_service, mock_cache, sample_raw_data
    ):
        """测试空字段列表"""
        mock_cache.get.return_value = sample_raw_data
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_invalid_fields(
        self, balance_service, mock_cache, sample_raw_data
    ):
        """测试无效字段"""
        mock_cache.get.return_value = sample_raw_data
        ...
        request = BalanceRequest(
            ts_code="600519.SH",
            fields=["end_date", "total_assets", "invalid_field", "another_invalid"],
        )
        ...
        assert response.status == ResponseStatus.SUCCESS
        selected_fields = set(response.data[0].keys())
        assert selected_fields == {"end_date", "total_assets"}
        assert "invalid_field" not in selected_fields
        assert "another_invalid" not in selected_fields

    @pytest.mark.asyncio
    async def test_get_balance_data_empty_result(
        self, balance_service, mock_cache, mock_tushare_source, sample_request
    ):
        """测试空数据结果"""
        mock_cache.get.return_value = None
        mock_tushare_source.get_balance_data.return_value = []  # 空结果
        ...
        assert response.message == "资产负债表查询成功，但无数据"

    @pytest.mark.asyncio
    async def test_get_balance_data_tushare_exception(
        self, balance_service, mock_cache, mock_tushare_source, sample_request
    ):
        """测试Tushare API异常"""
        mock_cache.get.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_concurrent_requests(
        self, balance_service, mock_cache, mock_tushare_source, sample_raw_data
    ):
        """测试并发请求"""
        ...
        request2 = BalanceRequest(
            ts_code="000001.SZ", fields=["end_date", "total_assets"]
        )

        # 并发执行
        responses = await asyncio.gather(
            balance_service.get_balance_data(request1),
            balance_service.get_balance_data(request2),
        )
    ...
    def test_generate_cache_key(self, balance_service, sample_request):
        """测试缓存键生成"""
        cache_key = balance_service._generate_cache_key(sample_request)
    ...
    def test_generate_cache_key_different_requests(self, balance_service):
        """测试不同请求生成不同的缓存键"""
        request1 = BalanceRequest(
            ts_code="600519.SH",
            fields=["end_date", "total_assets"],
        ...
        request2 = BalanceRequest(
            ts_code="600519.SH",
            fields=["end_date", "total_liab"],  # 不同字段
            start_date="20240101",
        )

        key1 = balance_service._generate_cache_key(request1)
        key2 = balance_service._generate_cache_key(request2)

        # 验证不同请求生成不同的缓存键
        assert key1 != key2

    @pytest.mark.asyncio
    async def test_service_health_check(self, balance_service, mock_tushare_source):
        """测试服务健康检查"""
        # 设置健康检查返回
...
class TestBalanceServiceEdgeCases(TestBalanceService):
    """测试BalanceService边界情况"""

    @pytest.mark.asyncio
    async def test_get_balance_data_malformed_data(
        self, balance_service, mock_cache, mock_tushare_source, sample_request
    ):
        """测试处理格式错误的数据"""
        # 设置格式错误的数据
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_large_dataset_performance(
        self, balance_service, mock_cache, sample_request
    ):
        """测试大数据集性能"""
        # 创建大数据集
        large_data = []
            ...
            large_data.append(
                {
                    ...
                    "end_date": "20240930",
                    "total_assets": 1000000000.0 + i * 1000000,
                    "total_liab": 500000000.0 + i * 500000,
                    "total_equity": 500000000.0 + i * 500000,
    ...
    @pytest.mark.asyncio
    async def test_get_balance_data_cache_key_collision(
        self,
        balance_service,
        mock_cache,
        mock_tushare_source,
