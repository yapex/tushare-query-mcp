...
class TestAsyncDiskCache:
    ...
    async def test_cache_set_and_get(self, cache_instance):
        ...
        await cache_instance.set("test_key", test_data, ttl=3600)

        # 获取缓存
        cached_data = await cache_instance.get("test_key")
    ...
    @pytest.mark.asyncio
    async def test_cache_overwrite(self, cache_instance):
        """测试缓存覆盖"""
        # 设置初始缓存
        initial_data = [{"initial": "data"}]
        await cache_instance.set("test_key", initial_data, ttl=3600)

        # 覆盖缓存
        new_data = [{"new": "data", "updated": True}]
        await cache_instance.set("test_key", new_data, ttl=3600)
    ...
    async def test_cache_ttl_expiration(self, cache_instance):
        ...
        # 设置短TTL的缓存
        test_data = [{"ttl": "test"}]
        await cache_instance.set("ttl_key", test_data, ttl=1)  # 1秒过期

        # 立即获取应该成功
        assert await cache_instance.get("ttl_key") == test_data

        # 等待过期
        time.sleep(1.1)

        # 过期后应该返回 None
        assert await cache_instance.get("ttl_key") is None
    ...
    async def test_cache_persistence(self, temp_cache_dir):
        ...
        # 验证数据存在
        assert await cache1.get("persistent_key") == test_data

        # 创建新的缓存实例（模拟重启）
        cache2 = AsyncDiskCache(cache_dir=temp_cache_dir)

        # 验证重启后数据依然存在
        persisted_data = await cache2.get("persistent_key")
    ...
    async def test_cache_clear(self, cache_instance):
        """测试缓存清理"""
        # 设置多个缓存
        await cache_instance.set("key1", "data1", ttl=3600)
        await cache_instance.set("key2", "data2", ttl=3600)
        await cache_instance.set("key3", "data3", ttl=3600)

        # 验证数据存在
        assert await cache_instance.get("key1") == "data1"
        assert await cache_instance.get("key2") == "data2"
        assert await cache_instance.get("key3") == "data3"
        ...
        assert await cache_instance.get("key3") is None

    @pytest.mark.asyncio
    async def test_cache_delete_key(self, cache_instance):
        """测试删除特定键"""
        # 设置多个缓存
        await cache_instance.set("keep_key", "keep_data", ttl=3600)
        await cache_instance.set("delete_key", "delete_data", ttl=3600)

        # 验证数据存在
        assert await cache_instance.get("keep_key") == "keep_data"
        assert await cache_instance.get("delete_key") == "delete_data"

        # 删除特定键
        await cache_instance.delete("delete_key")

        # 验证只有特定键被删除
        assert await cache_instance.get("keep_key") == "keep_data"
        assert await cache_instance.get("delete_key") is None
    ...
    @pytest.mark.asyncio
    async def test_cache_complex_data_types(self, cache_instance):
        """测试复杂数据类型的缓存"""
        # 测试各种数据类型
        test_cases = [
            "simple_string",
        ...
        for i, test_data in enumerate(test_cases):
            key = f"test_key_{i}"
            await cache_instance.set(key, test_data, ttl=3600)
            retrieved_data = await cache_instance.get(key)
    ...
    async def test_concurrent_cache_access(self, cache_instance):
        ...
        async def worker(worker_id):
            ...
            await cache_instance.set(key, data, ttl=3600)
