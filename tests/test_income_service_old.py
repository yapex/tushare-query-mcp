...
class TestIncomeService:
    ...
    def sample_request(self):
        ...
        return IncomeRequest(
            ts_code="600519.SH",
            fields=["end_date", "n_income_attr_p", "revenue", "basic_eps"],
            start_date="20240101",
            end_date="20241231",
        )
    ...
    @pytest.mark.asyncio
    async def test_get_income_data_cache_hit(
        self, income_service, mock_cache, sample_raw_data, sample_request
    ):
        """测试缓存命中场景"""
        # 设置缓存返回数据
        ...
        # 验证响应
        assert isinstance(response, IncomeResponse)
        assert response.status == ResponseStatus.SUCCESS
        assert response.from_cache is True
        assert response.data_type == "income"
        ...
        assert selected_record["n_income_attr_p"] == 19223784414.08

        # 验证查询时间很短（缓存命中）
        assert response.query_time < 0.01

    @pytest.mark.asyncio
    async def test_get_income_data_cache_miss(
        self,
        income_service,
        mock_cache,
        mock_tushare_source,
        ...
        # 验证数据源被调用
        mock_tushare_source.get_income_data.assert_called_once()
    ...
    async def test_get_income_data_with_update_flag_filtering(
        self, income_service, mock_cache, mock_tushare_source
    ):
        """测试update_flag过滤功能"""
        # 设置包含重复记录的原始数据
        ...
        request = IncomeRequest(
            ts_code="600519.SH", fields=["end_date", "n_income_attr_p", "update_flag"]
        )
    ...
    async def test_get_income_data_field_selection(
        ...
        assert response.data[0]["revenue"] == 120714458386.98

    @pytest.mark.asyncio
    async def test_get_income_data_no_dates(
        self, income_service, mock_cache, sample_raw_data
    ):
        """测试不提供日期范围"""
        mock_cache.get.return_value = sample_raw_data
    ...
    async def test_get_income_data_invalid_fields(
        ...
        request = IncomeRequest(
            ts_code="600519.SH",
            fields=["end_date", "n_income_attr_p", "invalid_field", "another_invalid"],
        )
        ...
        assert response.status == ResponseStatus.SUCCESS
        selected_fields = set(response.data[0].keys())
        assert selected_fields == {"end_date", "n_income_attr_p"}
    ...
    async def test_get_income_data_empty_result(
        self, income_service, mock_cache, mock_tushare_source, sample_request
    ):
        """测试空数据结果"""
        mock_cache.get.return_value = None
    ...
    async def test_get_income_data_concurrent_requests(
        self, income_service, mock_cache, mock_tushare_source, sample_raw_data
    ):
        """测试并发请求"""
        ...
        request1 = IncomeRequest(
            ts_code="600519.SH", fields=["end_date", "n_income_attr_p"]
        )
        request2 = IncomeRequest(
            ts_code="000001.SZ", fields=["end_date", "n_income_attr_p"]
        )
    ...
    async def test_get_income_data_with_report_type(
        ...
        request = IncomeRequest(
            ts_code="600519.SH",
            fields=["end_date", "n_income_attr_p", "report_type"],
            report_type="1",  # 添加报告类型
        )
        ...
        assert response.status == ResponseStatus.SUCCESS
        selected_fields = set(response.data[0].keys())
        assert "report_type" in selected_fields

    def test_generate_cache_key(self, income_service, sample_request):
        """测试缓存键生成"""
        cache_key = income_service._generate_cache_key(sample_request)
    ...
    def test_generate_cache_key_different_requests(self, income_service):
        """测试不同请求生成不同的缓存键"""
        request1 = IncomeRequest(
            ts_code="600519.SH",
            fields=["end_date", "n_income_attr_p"],
        ...
        request2 = IncomeRequest(
            ts_code="600519.SH",
            fields=["end_date", "revenue"],  # 不同字段
            start_date="20240101",
        )

        key1 = income_service._generate_cache_key(request1)
        key2 = income_service._generate_cache_key(request2)
...
class TestIncomeServiceEdgeCases(TestIncomeService):
    ...
    async def test_get_income_data_large_dataset_performance(
        self, income_service, mock_cache, sample_request
    ):
        """测试大数据集性能"""
        # 创建大数据集
        large_data = []
    ...
    @pytest.mark.asyncio
    async def test_get_income_data_cache_key_collision(
        self,
        income_service,
        mock_cache,
        mock_tushare_source,
