...
from ...schemas.request import CashFlowRequest
...
async def get_cashflow_data(
    ...
    fields: str = Query(
        ..., description="字段列表，逗号分隔（如：end_date,net_cashflows_act）"
    ),
    ...
    Args:
        ...
        start_date: 开始日期（可选）
        end_date: 结束日期（可选）
        service: CashFlowService实例（依赖注入）
    ...
    try:
        ...
        response = await service.get_cashflow_data(request)
...
async def health_check(service: CashFlowService = Depends(get_cashflow_service)):
    """
    健康检查
